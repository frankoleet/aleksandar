...
const Chip = ({ children }) => (
  <span className="inline-flex items-center rounded-md border border-cyan-400/15 bg-cyan-500/10 px-3 py-1.5 text-sm text-cyan-100/80">
    {children}
  </span>
);

const Section = ({ icon: Icon, title, subtitle, children }) => (
  <section className="rounded-2xl border border-cyan-400/10 bg-[#041a1f]/60 p-5 shadow-[0_0_0_1px_rgba(34,211,238,0.05),0_18px_60px_rgba(0,0,0,0.6)] backdrop-blur">
...
const data = {
  ...
  links: [
    ...
    { label: "Portfolio", value: "Here", href: "#" },
  ],
};
  ...
  return (
    <div className="min-h-screen bg-[#020d10] text-white">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 15% 10%, rgba(6,182,212,0.35) 0, rgba(0,0,0,0) 45%), radial-gradient(circle at 85% 35%, rgba(20,184,166,0.25) 0, rgba(0,0,0,0) 55%), radial-gradient(circle at 50% 95%, rgba(34,211,238,0.18) 0, rgba(0,0,0,0) 45%)",
        }}
      />
      <div className="pointer-events-none fixed inset-0 bg-[linear-gradient(to_bottom,rgba(6,182,212,0.04),transparent_30%,transparent_70%,rgba(20,184,166,0.03))]" />

      {/* HEADER */}
      <header className="relative mx-auto w-full max-w-screen-2xl px-12 pb-6 pt-10 print:pt-4">
        <motion.div
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col gap-2 md:items-end print:hidden">
              <div className="flex w-full gap-2 md:w-auto">
                <button
                  onClick={handleCopy}
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl border border-cyan-400/15 bg-cyan-500/5 px-5 py-2.5 text-base font-medium text-white/90 hover:bg-cyan-500/15 md:flex-none"
                >
                ...
                <button
                  onClick={handlePrint}
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl border border-cyan-400/30 bg-cyan-500/15 px-5 py-2.5 text-base font-medium text-cyan-100 hover:bg-cyan-500/25 md:flex-none"
                >
                  ...
                  {data.links.map((l) => (
                    <div key={l.label} className="flex items-center justify-between gap-4">
                      ...
                      <a
                        ...
                        className="truncate text-cyan-100/75 hover:text-cyan-300 transition-colors"
                      >
                        {l.value}
                      </a>
                    </div>
      ...
      {/* MAIN */}
      <main className="relative mx-auto w-full max-w-screen-2xl grid grid-cols-1 gap-4 px-12 pb-16 md:grid-cols-12 print:pb-4">
          ...
          <Section icon={Briefcase} title="Опыт" subtitle="Путь в информационной безопасности">
            <div className="space-y-4">
              {data.experience.map((e) => (
                <div key={e.title} className="rounded-2xl border border-cyan-400/10 bg-cyan-500/5 p-4">
          ...
          <Section icon={Sparkles} title="Проекты" subtitle="Практические работы и исследования">
            <div className="mb-4 flex items-center gap-2 rounded-2xl border border-cyan-400/10 bg-cyan-500/5 px-3 py-2 print:hidden">
              ...
              <input
                ...
                placeholder="Поиск по проектам (Python, GitHub, Linux...)"
                className="w-full bg-transparent text-base text-white/80 outline-none placeholder:text-cyan-200/35"
              />
            </div>
            <div className="space-y-4">
              {filteredProjects.map((p) => (
                <div key={p.name} className="rounded-2xl border border-cyan-400/10 bg-cyan-500/5 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 rounded-xl border border-cyan-400/15 bg-[#020d10]/40 px-3 py-1.5 text-sm text-cyan-200/65 hover:bg-[#020d10]/60"
                    >
              ...
              {filteredProjects.length === 0 && (
                <div className="rounded-2xl border border-cyan-400/10 bg-cyan-500/5 p-4 text-base text-cyan-100/65">
                ...
                <div className="mt-4 grid grid-cols-1 gap-2">
                  <div className="rounded-xl border border-cyan-400/10 bg-[#020d10]/40 p-3">
                    ...
                    <div className="mt-1 text-base text-white/80">
                      Понимаю атаки изнутри — это даёт преимущество при проектировании защитных систем.
                    </div>
                  </div>
                  <div className="rounded-xl border border-cyan-400/10 bg-[#020d10]/40 p-3">
                    <div className="text-sm font-semibold text-cyan-300/65">Сейчас прокачиваю</div>
                    <div className="mt-1 text-base text-white/80">
                      Сетевая безопасность, IAM-практики, углубление Python для автоматизации ИБ-задач.
                    </div>
                  </div>
                </div>
              </div>
          ...
          <Section icon={GraduationCap} title="Образование" subtitle="Академическая база">
            <div className="space-y-4">
              {data.education.map((ed) => (
                <div key={ed.title} className="rounded-2xl border border-cyan-400/10 bg-cyan-500/5 p-4">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    <div>
      ...
      {/* FOOTER */}
      <footer className="relative mx-auto w-full max-w-screen-2xl px-12 pb-10 text-sm text-cyan-200/40 print:hidden">
      ...
      {/* TOAST */}
      {toast && (
        <div className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 rounded-2xl border border-cyan-400/15 bg-[#020d10]/90 px-4 py-2 text-base text-cyan-100/80 shadow-xl backdrop-blur">
          {toast}
        </div>
      )}
